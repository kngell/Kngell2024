...
ADD INDEX idx_va_deleted_at (deleted_at);

ALTER TABLE product_variation
ADD COLUMN deleted_at TIMESTAMP NULL DEFAULT NULL AFTER updated_at,
ADD INDEX idx_pv_deleted_at (deleted_at);

ALTER TABLE variation_attribute ADD INDEX idx_va_deleted_at (deleted_at);
...
INSERT INTO
    variation_type (name, description)
VALUES
    (
        'size',
        'Defines physical size variants like S, M, L, XL'
    ),
    ('color', 'Defines color variations for a product'),
    (
        'material',
        'Defines material composition (cotton, leather, etc.)'
    ),
    (
        'capacity',
        'Defines storage or volume capacity (e.g. 20L, 1TB)'
    ),
    (
        'flavor',
        'Defines taste variants for consumable products'
    ),
    (
        'storage',
        'Defines storage capacity for electronics (e.g. 64GB, 128GB)'
    ),
    ('ram', 'Defines RAM size for electronic devices'),
    (
        'license_type',
        'Defines type of software license'
    ),
    (
        'duration',
        'Defines duration or validity of a digital product'
    );
