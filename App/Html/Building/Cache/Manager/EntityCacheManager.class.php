...
final class EntityCacheManager
{
    ...
    private function reorderEntitiesByIdentifiers(array $entities, array $orderedIdentifiers): array
    {
        ...
        // If both arrays have the same count, we can skip if they're already in order
        if (count($entities) === count($orderedIdentifiers)) {
            // Optional: Check if already ordered
            $firstEntityId = $this->keyGenerator->getCacheIdentifier($entities[0]);
            if ($firstEntityId === $orderedIdentifiers[0]) {
                return $entities; // Already in correct order
            }
        }
