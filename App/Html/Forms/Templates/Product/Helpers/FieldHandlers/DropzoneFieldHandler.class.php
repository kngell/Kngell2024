...
class DropzoneFieldHandler implements FieldHandlerInterface
{
    ...
    private function createPreviewItem(FormBuilder $form, AbstractForm $formInstance, array $field, array $fileInfo, string $webPath): AbstractHtmlComponent
    {
        ...
        // $displayWebPath = $this->getOriginalWebPath($webPath, $fieldName);

        // $submissionWebPath = $this->generateFieldSpecificWebPath($webPath, $fieldName);

        // Generate unique hidden input name
    ...
    // private function getOriginalWebPath(string $fieldSpecificWebPath, string $fieldName): string
    // {
    //     $pathInfo = pathinfo($fieldSpecificWebPath);
    //     $fieldPrefix = $this->getFieldPrefix($fieldName) . '_';

    //     $originalFilename = $pathInfo['basename'];

    //     while (str_starts_with($originalFilename, $fieldPrefix)) {
    //         $originalFilename = substr($originalFilename, strlen($fieldPrefix));
    //     }
    ...
    //     return $pathInfo['dirname'] . '/' . $originalFilename;
    // }

    /**
     * Generate a field-specific web path to prevent cross-contamination.
     */
    // DropzoneFieldHandler.php

    // private function generateFieldSpecificWebPath(string $originalWebPath, string $fieldName): string
    // {
    //     $pathInfo = pathinfo($originalWebPath);
    //     $fieldPrefix = $this->getFieldPrefix($fieldName) . '_';
    //     $filename = $pathInfo['basename'];

    //     // ➡️ MODIFICATION: Only add the prefix if it's NOT already there.
    //     if (!str_starts_with($filename, $fieldPrefix)) {
    //         $filename = $fieldPrefix . $filename;
    //     }
    ...
    /**
     * Get a unique prefix for each field.
     */
    private function getFieldPrefix(string $fieldName): string
    {
        $prefixes = [
            'main_image' => 'main_img',
            'img_gallery' => 'gallery',
        ...
        // Handle array fields like 'img_gallery[]'
        $cleanFieldName = rtrim($fieldName, '[]');

        return $prefixes[$cleanFieldName] ?? 'file';
    }
     ...
     */
    private function isImageFile(?string $mimeType, string $fileName): bool
    {
        $imageMimes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml'];
        $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'];

        if ($mimeType && in_array($mimeType, $imageMimes)) {
            return true;
        }
