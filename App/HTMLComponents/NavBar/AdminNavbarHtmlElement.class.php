...
class AdminNavbarHtmlElement extends AbstractNavbarHtmlElement
{
    ...
    public function display(): string
    {
        ...
        return $nav->tag('li')->class('nav-item dropdown')->add(
            $nav->tag('a')->class('nav-link dropdown-toggle')->href('#')->role('button')->dataBsToggle('dropdown')->ariaExpanded(false)->add(
                $nav->tag('img')->alt('image')->src($this->media($user->getMedia()))->class('rounded-circle-custom'),
            ),
            $nav->tag('ul')->class('dropdown-menu dropdown-menu-end')->add(
                $nav->tag('li')->add(
                    $nav->tag('a')->class('dropdown-item')->href('/admin/dashboard/edit-profile')->add(
                        $nav->tag('i')->class('fa-solid fa-user')->content('Edit Profile')
                    )
                ),
                $nav->tag('li')->add(
                    $nav->tag('a')->class('dropdown-item')->href('/logout')->add(
                        $nav->tag('i')->class('fa-solid fa-sign-out-alt')->content('Logout')
                    )
                )
            )
        )->generate();
