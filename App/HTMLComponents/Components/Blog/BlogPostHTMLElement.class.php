...
class BlogPostHTMLElement
{
    ...
    public function display(): string
    {
        ...
        foreach ($this->posts as $post) {
            ...
            $posts[] = $html->htmlTag('div')->class($this->wrapperClass)->style($this->wrapperStyle)->add(
                $html->htmlTag('div')->class(['card-body'])->add(
                    $html->htmlTag('h3')->content($this->htmlDecode($post->getTitle(), ENT_QUOTES)),
                    $html->htmlTag('p')->content($this->getContentOverview($this->htmlDecode($post->getTitle()))),
                    $html->htmlTag('a')->href("/post/show/{$post->getPostId()}")->class(['btn btn-primary'])->content('Show Post'),
                    $html->htmlTag('a')->href("/post/edit/{$post->getPostId()}")->class(['btn btn-info'])->content('Edit Post'),
