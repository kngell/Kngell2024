...
final class EnumType implements TypeHandlerInterface
{
    ...
    public function normalizeForEntity(mixed $value, ReflectionProperty $property, object $contextEntity): mixed
    {
        ...
        // 2.5. Handle empty strings BEFORE checking enum type
        if (is_string($value) && $value === '') {
            return $this->handleEmptyStringValue($property, $enumClass);
        }
    ...
    private function handleEmptyStringValue(ReflectionProperty $property, string $enumClass): ?object
    {
        if ($property->getType()?->allowsNull()) {
            return null;
        }
        ...
        if ($property->hasDefaultValue()) {
            ...
            if (is_object($default) && $default instanceof $enumClass) {
                return $default;
            }
        }

        // Look for a case named 'unknown', 'default', or similar
        foreach ($cases as $case) {
            $caseName = strtolower($case->name);
            if (in_array($caseName, ['unknown', 'default', 'not_set', 'notspecified', 'unspecified'])) {
                return $case;
            }
        }

        // Return the first case as fallback
        if (!empty($cases)) {
            return $cases[0];
        }

        // No cases available - this shouldn't happen but handle it
        throw new InvalidArgumentException(
            "Empty value for enum {$enumClass} with no cases on property {$property->getName()}",
        );
    }
    ...
    private function handleBackedEnum(mixed $value, string $enumClass, ReflectionProperty $property): object
    {
        ...
        try {
            ...
            // Try to normalize the value (trim whitespace, etc.)
            if (is_string($value)) {
    ...
    private function safeStringConversion(mixed $value): string
    {
        ...
        // Allow null to convert to empty string (for consistency)
        if ($value === null) {
            return '';
        }
    ...
    private function tryCaseInsensitiveMatch(string $enumClass, string $value): ?object
    {
        ...
        foreach ($enumClass::cases() as $case) {
            ...
            if ($case instanceof BackedEnum) {
                $caseName = $this->normalizeStringValue($case->name);
                $caseValue = $this->normalizeStringValue((string) $case->value);

                if ($caseName === $normalizedValue || $caseValue === $normalizedValue) {
                    return $case;
                }
            } else {
                // For pure enums, check only name
                $caseName = $this->normalizeStringValue($case->name);
                if ($caseName === $normalizedValue) {
                    return $case;
                }
            }
        }
