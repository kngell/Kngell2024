...
class FooterLinkSaveService extends AbstractSaveService
{
    ...
    {
        return new FooterMenuLinkEvent($eventData);
    }

    public function getEditUrl(int $entityId): string
    {
        return "/admin/footer-link/{$entityId}/edit";
    }
    ...
    {
        return FooterMenuLinkEvent::class;
    }
    ...
    public function getSuccessMessage(string $operationType, bool $wasSkipped): string
    {
        return match($operationType) {
            'insert' => 'Footer Link has been created successfully',
            'update' => !$wasSkipped
                ? 'Footer Link has been updated successfully'
                : 'No changes were made to the Footer Column',
            'delete' => 'Footer Link has been deleted successfully',
