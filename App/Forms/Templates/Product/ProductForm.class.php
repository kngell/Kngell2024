...
final readonly class ProductForm extends AbstractForm
{
    ...
    /**
     * Get form sections with dynamic options populated.
     */
    protected function getFormSections(): array
    {
        $sections = self::FORM_SECTIONS;

        // Populate dynamic options for variation type
        $sections['variation'][0]['content'][0]['options'] = $this->getVariationTypeOptions();
        $sections['variation'][0]['content'][5]['options'] = $this->getStockStatusOptions();
    ...
    private function buildVariationGroups(bool $isEdit, array|Entity|bool $formValues): array
    {
        // Template for one variation group (we’ll duplicate this)
        $baseGroup = $this->getBaseVariationGroup();

        if (!$isEdit) {
            // New product → show one empty variation
            return [$this->buildVariationGroup(0, [])];
        }

        // Existing product → load variations from DB
        $productId = $formValues instanceof Entity
            ? $formValues->getId()
            : ($formValues['id'] ?? null);
        ...
        $variations = $model->findByProductId($productId)->asArray(); // or asClass()
        ...
        foreach ($variations as $index => $variation) {
            $groups[] = $this->buildVariationGroup($index, $variation);
        }
