...
export default class MediaUpload {
  ...
  bindExistingItems() {
    const existingItems = this.preview.querySelectorAll(".media-preview__item");
    existingItems.forEach((item) => {
      const removeBtn = item.querySelector(".media-preview__item--icon-remove");
      if (removeBtn) {
        removeBtn.addEventListener("click", (e) => {
          e.preventDefault();
          this.removeMediaItem(item, null);
        });
      }
    });
  }
  ...
  removeMediaItem(item, objectURL) {
    logger.debug("🗑️ Removing item:", item.dataset.filename);

    // 1. Handle Memory (for new uploads)
    if (objectURL) URL.revokeObjectURL(objectURL);

    // 2. Handle Database/Existing Files
    // If there is a hidden input, remove it from the DOM so it's not sent in the POST request
    const existingInput = item.querySelector(".existing-media-path");
    if (existingInput) {
      logger.debug("Removing existing database reference");
      existingInput.remove();
    }

    // 3. Handle Local File Input (for new uploads)
