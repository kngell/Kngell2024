...
abstract class AbstractCacheStorage implements IterableStorageInterface
{
    ...
    {
        $cachePath = $this->cacheDirectory . $entryIdentifier . $this->cacheEntryFileExtension;

        // Ensure parent directory exists using DirectoryManager
        $parentDirectory = dirname($cachePath);
        if (!$this->directoryManager->exists($parentDirectory)) {
            $this->directoryManager->create($parentDirectory, 0775);
        }
