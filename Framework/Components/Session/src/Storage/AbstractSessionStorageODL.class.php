...
abstract class AbstractSessionStorageOLD
{
    use SessionTrait;
    ...
    {
        try {
            $path = ROOT_DIR . DS . $this->sessionEnvironment->storagePath() . DS;
            if (session_status() == PHP_SESSION_NONE) {
                session_save_path(ROOT_DIR . DS . $this->sessionEnvironment->storagePath() . DS);
                session_start();
            }
        } catch (RuntimeException $th) {
            throw            new RuntimeException('Could not start the session. ' . $th->getMessage());
        }
    }
    ...
    {
        // FIX: Prevent 'ini_set(): Session ini settings cannot be changed when a session is active'
        // If a session is already active (started), we must not attempt to change INI settings.
