...
abstract class AbstractFileUploadServiceOld implements FileUploadComponentInterface, UploadMediapathsInterface
{
    ...
    public function cleanupOldTempFiles(): int
    {
        if (!isset($this->tempFileCleaner)) {
            return $this->manualCleanupOldTempFiles();
        }

        return $this->tempFileCleaner->clean($this->getTempDirectory());
    }
    ...
    public function makePermanent(): bool
    {
        ...
        try {
            ...
            $this->cleanupProcessedFiles($tempPaths);
    ...
    {
        $storagePath = realpath(STORAGE) . DS;
        // Ensure target directory has a consistent slash for calculation
        $targetDir = rtrim($this->getTargetDirectory(), DS) . DS;
        ...
        if (str_starts_with($targetDir, $storagePath)) {
            ...
            return '/' . trim(str_replace(DS, '/', $relative), '/') . '/';
        }
    ...
    private function manualCleanupOldTempFiles(): int
    {
        $cleanedCount = 0;
        $now = time();
        $tempDir = $this->getTempDirectory();
