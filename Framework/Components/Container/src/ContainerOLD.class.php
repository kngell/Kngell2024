 ...
 */
class ContainerOLD implements ContainerInterface
{
    ...
    public function taggedLazy(string $tag): LazyTagCollection
    {
        if (!isset($this->tags[$tag])) {
            return new LazyTagCollection($this, $tag, []);
        }

        return new LazyTagCollection($this, $tag, $this->tags[$tag]);
    }
    ...
    protected function buildClass(string $concrete, array $parameters = [], ?BindingDefinition $binding = null): object
    {
        ...
        foreach ($constructor->getParameters() as $index => $parameter) {
            ...
            if ($parameter->isVariadic()) {
                // If it's a LazyTagCollection, convert to array of resolved services
                // but only when actually used in the method
                if ($resolvedValue instanceof LazyTagCollection) {
                    // You could either resolve all now:
                    $args = array_merge($args, $resolvedValue->all());
                    // Or keep it lazy by passing the collection itself
                    // but then the consuming class needs to handle it
    ...
    protected function resolveDependency(ReflectionParameter $parameter, array $parameters = []): mixed
    {
        ...
        if ($type instanceof ReflectionNamedType) {
            ...
            if ($parameter->isVariadic()) {
                ...
                // Use the interface name itself as the tag
                if (!empty($this->tags[$typeName] ?? [])) {
                    // Return LAZY collection instead of resolved instances
                    return $this->taggedLazy($typeName);
                }

                // Optionally: fallback to inferred string tag
                $tagName = $this->inferTagFromType($typeName);
                if (!empty($this->tags[$tagName] ?? [])) {
                    return $this->taggedLazy($tagName);
                }
