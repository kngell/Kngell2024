...
class WhereRules extends AbstractRules implements QueryRulesInterface
{
    ...
    protected function normalize(array $conditions): array
    {
        ...
        foreach ($conditions as $condition) {
            ...
            if ($this->isInCondition($conditions) && !str_contains(strtolower($this->method), 'in')) {
                $this->method = 'whereIn';
            }
    ...
    private function isSqlListString(mixed $value): bool
    {
        return is_string($value)
            && preg_match('/^\s*\([^()]+\)\s*$/', $value);
    }

    private function isInCondition(array $condition): bool
    {
        $count = count($condition);

        if ($count === 1 && isset($condition[0]) && is_array($condition[0])) {
            $condition = $condition[0];
            $count = count($condition);
        }
        if ($count === 1) {
            $col = array_key_first($condition);
            $val = $condition[$col];

            return is_string($col)
                && (is_array($val) || $this->isSqlListString($val));
        }

        // ['col', value]
        if ($count === 2) {
            return is_string($condition[0])
                && (is_array($condition[1]) || $this->isSqlListString($condition[1]));
        }
        ...
        if ($count === 3) {
            return is_string($condition[0])
                ...
                && in_array(
                    strtoupper(trim($condition[1])),
                    ['IN', 'NOT IN'],
                    true,
                )
                && (is_array($condition[2]) || $this->isSqlListString($condition[2]));
        }
