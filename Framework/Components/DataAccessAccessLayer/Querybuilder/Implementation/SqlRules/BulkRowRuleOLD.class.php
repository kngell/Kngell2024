...
class BulkRowRuleOLD extends AbstractRules implements QueryRulesInterface
{
    ...
    public function getRule(array $rowValuesData): string
    {
        ...
        foreach ($normalized as $rowIndex => $item) {
            ...
            foreach ($columnNames as $colIndex => $column) {
                ...
                if ($this->usePreparedRows) {
                    $orderedRow[] = ':' . $this->createParameter($val, $column, $helper, $colIndex, $entity);
                } else {
    ...
    {
        // $isObjectCollection = $emData instanceof CollectionInterface || !ArrayUtils::isArrayList($emData) || ArrayUtils::isObjectList($emData);
