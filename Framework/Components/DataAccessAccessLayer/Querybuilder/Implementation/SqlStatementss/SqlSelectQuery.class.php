...
class SqlSelectQuery extends SqlQuery implements SqlSelectQueryBuilderInterface
{
    ...
    private const SqlStatementType TYPE = SqlStatementType::SELECT;
    ...
    private null|string|closure $currentTable = null;
    ...
    public function from(null|string|Closure $table = null, ?string $alias = null): self
    {
        ...
        list($table, $key) = $this->getUniqueTableName(__FUNCTION__, $this->table, $this->queryMap);

        $this->customAlias = $alias;

        $this->table = $table;
        $this->selectMap['select']['table'] = $table;
    ...
    {
        list($table, $key) = $this->getUniqueTableName(__FUNCTION__, $this->table, $query->getQueryMap());
