...
class ConditionGroupOld extends SqlQuery implements ClauseComponentInterface, OperatorAwareInterface
{
    ...
    public function hasOrOperators(): bool
    {
        foreach ($this->children->all() as $child) {
            if ($child instanceof ConditionClause) {
                $op = SqlOperator::tryFrom($child->getLogicalLink());
    ...
    private function needsGroupParentheses(): bool
    {
        ...
        if ($childCount <= 1) {
            ...
            if ($firstChild instanceof ConditionGroup) {
                return $firstChild->needsGroupParentheses();
            }
            return false;
        }
        ...
        foreach ($this->children->all() as $child) {
            ...
            if ($child instanceof ConditionClause) {
                $logicalLink = $child->getLogicalLink();
            } elseif ($child instanceof ConditionGroup) {
                $logicalLink = $child->getGroupLink();
            }

            $op = SqlOperator::tryFrom($logicalLink);
