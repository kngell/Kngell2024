...
final class TypeHandlerFactory
{
    ...
    public function getHandlerForValue(mixed $value, ?ReflectionProperty $property = null): TypeHandlerInterface
    {
        ...
        // STRATEGY 1: First handle NULL values (they have highest priority)
        if ($value === null) {
            return $this->valueBasedHandlers['null'];
        }

        // STRATEGY 2: Then try TYPE-BASED matching (most reliable for enums, objects, etc.)
        if ($property !== null) {
        ...
        // STRATEGY 3: Then try VALUE-BASED matching (good for arrays, dates, etc.)
    ...
    {
        // Try to find the most specific handler for the actual value
    ...
    {
        // Skip null check since it's handled above
        foreach ($this->valueBasedHandlers as $name => $handler) {
            if ($name === 'null') {
    ...
    {
        // These handlers are selected based on the ACTUAL VALUE
        $this->valueBasedHandlers = [
            'null' => new NullType(),           // Handles null values
            'array' => new ArrayType(),         // Handles arrays and JSON strings
            'datetime' => new DateTimeType(),   // Handles DateTime objects and date strings
            'standard' => new StandardType(),   // Handles all scalar values (fallback)
        ];
    }

    private function initializeTypeBasedHandlers(): void
    {
        // These handlers are selected based on PROPERTY TYPE
        $this->typeBasedHandlers = [
            // Scalar types (normalized)
            ...
            // Special types - ENUM IS CRITICAL
            'enum' => new EnumType(),
            'object' => new ObjectType(),
        ];

        // Money handler - lazy load to avoid circular deps
