...
class QueryResultFormatterOLD
{
    ...
    public function asClass(?string $entityClass = null): mixed
    {
        ...
        // Check if we need cartesian handling
        if ($this->requiresCartesianHandling($entityClass)) {
    ...
    private function requiresCartesianHandling(string $entityClass): bool
    {
        ...
        return $this->cartesianDetector->isSingleEntityWithJoins(
            $this->queryResult->getQeuryString(),
            $this->queryResult->getOperation(),
            $primaryKeyField,
        );
    }
