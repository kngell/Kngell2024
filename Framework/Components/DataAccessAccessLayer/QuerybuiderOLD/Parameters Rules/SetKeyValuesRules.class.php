...
class SetKeyValuesRules extends AbstractConditionRules
{
    ...
    public function getRule(?array $inputValues): string
    {
        $entity = $this->em->getEntity();

        $dirtyData = !empty($inputValues)
            ? $inputValues
            : $entity->getDirtyData();
        ...
        $normalizedData = $this->normalizer->normalizeValuesForDatabase($dirtyData, $entity);
        ...
        foreach ($normalizedData as $propertyName => $value) {
            ...
            $paramName = $this->paramPrefix($dbFieldName) . '_' . $dbFieldName;

            $ruleParts[] = $dbFieldName . ' = :' . $paramName;
